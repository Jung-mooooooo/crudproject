...
<tbody>
      ...
      <td><input type="text" v-model="newRow.hpbbyNo" disabled style="width: 50px"/></td>
      <td><input type="text" v-model="newRow.hobbyName" style="width: 100px"/></td>
      <td><input type="text" v-model="newRow.hobbyCat" style="width: 100px"/></td>
    ...
    <tr v-for="(row, index) in rows" :key="row.hobbyNo">
      <td>{{row.hobbyNo}}</td>
      <td v-show="!editMode">{{row.hobbyName}}</td>
      <td v-show="editMode"><input type="text" v-model="rows[index].hobbyName" style="width:100px"/></td>
      <td v-show="!editMode">{{row.hobbyCat}}</td>
      <td v-show="editMode"><input type="text" v-model="rows[index].hobbyCat" style="width:100px"/></td>
  ...
  export default {
    ...
    methods: {
      fetchWelfareFacility() {
        ...
        this.$axios.get('/welfareProgram/hobbyList', {
    ...
    saveNewRow() {
        this.rows.unshift({...this.newRow, hobbyNo: this.rows[0].hobbyNo + 1});
        this.showNewRow = false;
        let apiUrl = '/hobby'
        this.form = {
            "hobbyNo": this.newRow.hobbyNo,
            "hobbyName": this.newRow.hobbyName,
            "hobbyCat": this.newRow.hobbyCat,
    ...
    deleteRow(index) {
        let hobbyNo = this.rows[index].hobbyNo;
        this.$axios.delete('/hobby/' + hobbyNo)
            .then(() => {
    ...
    toggleEditMode() {
        ...
        if (this.editMode) {
            ...
            this.$axios.patch('/hobby', this.form)
        }
